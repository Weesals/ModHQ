...
namespace RTS4.Data {
    ...
    public class BTXImage : RTS3Image {
        ...
        public static BTXImage Load(Stream file) {
            var reader = new BinaryReader(file);
            string header = "";
            for (int h = 0; h < 3; ++h) header += reader.ReadChar();
            if (header != "btx") throw new InvalidDataException();
            int ver = reader.ReadInt16();
            int wut = reader.ReadInt32();
            int iw = reader.ReadInt16();
            int ih = reader.ReadInt16();
            var img = new BTXImage();
            img.Header = new ImageHeader() {
                Width = iw,
                Height = ih,
                Format = ImageHeader.FormatE.Uncompressed32,
            };
            img.PixelData = reader.ReadBytes(iw * ih * 4);
            int acount = 0;
            for (int p = 0; p < img.PixelData.Length / 4; ++p) acount += img.PixelData[p * 4 + 3];
            if (acount == 0) {
                for (int p = 0; p < img.PixelData.Length / 4; ++p) img.PixelData[p * 4 + 3] = 255;
            }
            return img;
        }
        ...
        public override byte[] Get32BitUncompressed() {
            ...
            switch (Header.Format) {
                case ImageHeader.FormatE.Uncompressed32: {
                    if (PixelData.Length != pixels.Length) return null;
