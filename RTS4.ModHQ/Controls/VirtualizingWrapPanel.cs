...
﻿    public class VirtualizingWrapPanel : VirtualizingPanel, IScrollInfo {
        ...
        protected override Size MeasureOverride(Size availableSize) {
            ...
            int itemsPerRow = (int)Math.Floor(realizedFrameSize.Width / ItemWidth);
            int visiblePerScreen = (!double.IsNaN(ItemHeight) && ItemHeight > 0 ?
                (int)Math.Ceiling((realizedFrameSize.Height - 1) / ItemHeight + 1) :
                0);
            using (_generator.StartAt(startPos, GeneratorDirection.Forward, true)) {
                ...
                currentY -= ChildSlotSize.Height * (VerticalOffset - currentSection);
                while (current < itemCount) {
                    ...
                    if (newlyRealized) {
                        if (child is FrameworkElement) {
                            ((FrameworkElement)child).HorizontalAlignment = System.Windows.HorizontalAlignment.Stretch;
                            ((FrameworkElement)child).VerticalAlignment = System.Windows.VerticalAlignment.Stretch;
                        }
                    ...
                    //if (stop)
                    if (current >= firstVisibleIndex + visiblePerScreen * itemsPerRow)
                        break;
                    current++;
                    childIndex++;
                }
            ...
            if (visiblePerScreen == 0) visiblePerScreen = visibleSections;

            ComputeExtentAndViewport(availableSize, visiblePerScreen);
